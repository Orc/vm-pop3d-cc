...
struct md5_ctx *ctx;
{
   ...
   * length of the file up to 2^64 bits.  Here we only compute the number
   * of bytes.  Do a double word increment.  */
  ctx->total[0] += len;
  if (ctx->total[0] < len)
    ++ctx->total[1];

  /* Process all bytes in the buffer with 64 bytes in each round of the
   * loop.  */
  while (words < endp) {
